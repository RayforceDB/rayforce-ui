...
struct grid_ui_state_t {
  ...
  float menu_pos[2];      // dropdown anchor position
  int editing_col;        // column index for rule builder, -1 = none
  bool adding_color_rule; // true = color rule, false = callback rule
  bool builder_focus_pending; // true when rule builder just opened
  // Rule builder state
  ...
  std::unordered_map<std::pair<int, int>, cell_change_t, pair_hash>
      cell_changes;
};
...
static void send_col_rules(rfui_widget_t *widget, i64_t col_idx) {
  if (!g_ctx || !widget || col_idx < 0 || col_idx >= MAX_COLS)
    return;
  ...
  rfui_col_rules_t *snapshot =
      (rfui_col_rules_t *)malloc(sizeof(rfui_col_rules_t));
  if (!snapshot) {
    free(msg);
    return;
  }
...
static i64_t get_cell_value(obj_p col, i64_t row) {
  if (!col || row < 0 || row >= col->len)
    return 0;
  switch (col->type) {
  case TYPE_I64:
  case TYPE_TIMESTAMP:
    return AS_I64(col)[row];
  case TYPE_I32:
  case TYPE_DATE:
  case TYPE_TIME:
    return AS_I32(col)[row];
  case TYPE_I16:
    return AS_I16(col)[row];
  case TYPE_F64: {
  ...
  case TYPE_SYMBOL:
    return AS_SYMBOL(col)[row];
  case TYPE_B8:
    return AS_B8(col)[row];
  case TYPE_U8:
    return AS_U8(col)[row];
  default:
    return 0;
  }
}
...
static void render_cell(obj_p col, i64_t row) {
  ...
  case TYPE_LIST: {
    ...
    if (item) {
      ImGui::TextDisabled("[%s:%lld]", type_name(item->type),
                          (long long)item->len);
    } else {
...
nil_t rfui_render_grid(rfui_widget_t *widget) {
  ...
  if (keys->type != TYPE_SYMBOL) {
    ImGui::TextDisabled("Table keys must be symbols (got %s)",
                        type_name(keys->type));
    return;
  }

  if (vals->type != TYPE_LIST) {
    ImGui::TextDisabled("Table values must be a list (got %s)",
                        type_name(vals->type));
    return;
  }
  ...
  for (i64_t col_idx = 1; col_idx < ncols; col_idx++) {
    ...
    if (col->len != nrows) {
      ImGui::TextDisabled("Column %lld length mismatch: %lld vs %lld",
                          (long long)col_idx, (long long)col->len,
                          (long long)nrows);
      return;
    }
  }

  // Initialize UI state if needed (must use new for C++ members like
  // unordered_map)
  ...
  if (!ui_state) {
    ...
    if (ui_state) {
      ...
      ui_state->menu_col = -1;
      ui_state->menu_open_request = false;
      ui_state->menu_pos[0] = 0.0f;
      ui_state->menu_pos[1] = 0.0f;
      ui_state->editing_col = -1;
      ui_state->adding_color_rule = true;
      ui_state->builder_focus_pending = false;
      ...
      ui_state->builder_pos[0] = 0.0f;
      ui_state->builder_pos[1] = 0.0f;
      ui_state->builder_expr[0] = '\0';
  ...
  if (ui_state && ui_state->selected_row >= 0) {
    ImGui::Text("Rows: %lld  Columns: %lld  Selected: %d", (long long)nrows,
                (long long)ncols, ui_state->selected_row);
    ImGui::SameLine();
    ImGui::PopStyleColor();
    ...
    ImGui::Text("Rows: %lld  Columns: %lld", (long long)nrows,
                (long long)ncols);
    ImGui::PopStyleColor();
  }
  ...
  if (ImGui::BeginTable("##grid", (int)ncols, table_flags, outer_size)) {
    ...
    // Custom header row — click opens column menu (rendered after EndTable)
    ...
    for (i64_t ci = 0; ci < ncols && ci < MAX_COLS; ci++) {
      ...
      snprintf(header_label, sizeof(header_label), "%s " ICON_CHEVRON_DN,
               col_name);
      ImGui::TableHeader(header_label);

      if (ui_state && ImGui::IsItemClicked(ImGuiMouseButton_Left)) {
        ui_state->editing_col = -1;
        ui_state->menu_col = (int)ci;
        ui_state->menu_open_request = true;
        ui_state->menu_pos[0] = ImGui::GetItemRectMin().x;
        ui_state->menu_pos[1] = ImGui::GetItemRectMax().y;
      }
    }
    ...
    // Cache column pointers for performance (avoid repeated AS_LIST
    // dereference)
      ...
      for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; row++) {
        ...
        for (i64_t col_idx = 0; col_idx < ncols; col_idx++) {
          ...
          for (int oi = 0; oi < ov_count; oi++) {
            ...
            if (ov->col_idx != -1 && ov->col_idx != col_idx)
              continue;
          ...
          if (flash_active) {
            ImU32 flash_color = ImGui::GetColorU32(
                ImVec4(0.2f, 0.8f, 0.3f, flash_alpha * 0.5f));
  ...
  // Column dropdown menu (simulated as floating window to prevent blocking)
  // We use a manual window instead of BeginPopup so clicks outside can interact
  // with other windows immediately.
  ...
  if (ui_state && ui_state->menu_open_request) {
    ...
    ImGui::SetNextWindowPos(
        ImVec2(ui_state->menu_pos[0], ui_state->menu_pos[1]));
    ImGui::OpenPopup("##col_menu");
  }
  ImGui::SetNextWindowSizeConstraints(ImVec2(220, 0), ImVec2(400, FLT_MAX));
  if (ui_state && ImGui::BeginPopup("##col_menu")) {

    i64_t ci = ui_state->menu_col;
    if (ci >= 0 && ci < (int)ncols && ci < MAX_COLS) {
      const char *col_name = str_from_symbol(AS_SYMBOL(keys)[ci]);
      if (!col_name)
        col_name = "?";

      ImGui::TextDisabled("%s", col_name);
      ImGui::Separator();
      ...
      if (ImGui::Selectable(ICON_SORT_UP " Sort Ascending")) {
        ...
        char sort_expr[256];
        snprintf(sort_expr, sizeof(sort_expr), "(fn [_d] (xasc ['%s] _d))",
                 col_name);
      ...
      if (ImGui::Selectable(ICON_SORT_DOWN " Sort Descending")) {
        ...
        widget->col_rules[ci].sort_dir = -1;
        char sort_expr[256];
        snprintf(sort_expr, sizeof(sort_expr), "(fn [_d] (xdesc ['%s] _d))",
                 col_name);
        send_post_query(widget, sort_expr);
      }
      ...
      if (widget->col_rules[ci].num_rules < MAX_COL_RULES) {
        if (ImGui::Selectable(ICON_PLUS " Add Color Rule...")) {
          ui_state->builder_pos[0] = ui_state->menu_pos[0];
          ui_state->builder_pos[1] = ui_state->menu_pos[1];
          ui_state->editing_col = (int)ci;
          ui_state->adding_color_rule = true;
          ui_state->builder_focus_pending = true;
          ui_state->builder_op = RFUI_OP_GT;
          ui_state->builder_value[0] = '\0';
          ui_state->builder_value2[0] = '\0';
          ui_state->builder_color[0] = 1.0f;
        ...
        if (ImGui::Selectable(ICON_PLUS " Add Expression Rule...")) {
          ui_state->builder_pos[0] = ui_state->menu_pos[0];
          ui_state->builder_pos[1] = ui_state->menu_pos[1];
          ui_state->editing_col = (int)ci;
          ui_state->adding_color_rule = false;
          ui_state->builder_focus_pending = true;
          ui_state->builder_expr[0] = '\0';
        }
      }
      ...
      if (widget->col_rules[ci].num_rules > 0) {
        ...
        for (int ri = 0; ri < cr->num_rules; ri++) {
          ...
          const char *op_s =
              (r->op >= 0 && r->op < op_count) ? op_names[(int)r->op] : "?";
  ...
  // Reset menu_col logic (we handled auto-close inside the window loop
  // manually) if (ui_state && ui_state->menu_col >= 0 &&
  // !ImGui::IsPopupOpen("##col_menu")) {
  //    ui_state->menu_col = -1;
  // }

  // Rule builder — standalone window (needs to persist after popup closes)
  if (ui_state && ui_state->editing_col >= 0 &&
      ui_state->editing_col < (int)ncols && ui_state->editing_col < MAX_COLS) {
    ...
    char win_title[64];
    snprintf(win_title, sizeof(win_title), "Add Rule - %s###RuleBuilder_%p",
             col_name, (void *)widget);

    bool open = true;
    ImGui::SetNextWindowPos(
        ImVec2(ui_state->builder_pos[0], ui_state->builder_pos[1]),
        ImGuiCond_Appearing);
    if (ui_state->builder_focus_pending) {
      ImGui::SetNextWindowFocus();
    }
    ImGui::SetNextWindowSize(ImVec2(320, 0), ImGuiCond_FirstUseEver);
    bool visible = ImGui::Begin(win_title, &open,
                                ImGuiWindowFlags_AlwaysAutoResize |
                                    ImGuiWindowFlags_NoCollapse);
    ui_state->builder_focus_pending = false;
    if (visible) {
      if (ui_state->adding_color_rule) {
        // --- Color Rule Builder ---
        // Operator selector (manual popup avoids combo popup quirks)
        ImGui::PushID("op_select");
        if (ImGui::Button(op_names[ui_state->builder_op], ImVec2(80, 0))) {
          ImGui::OpenPopup("##op_menu");
        }
        ImVec2 op_popup_pos = ImGui::GetItemRectMin();
        op_popup_pos.y = ImGui::GetItemRectMax().y;
        ImGui::SetNextWindowPos(op_popup_pos, ImGuiCond_Appearing);
        if (ImGui::BeginPopup("##op_menu")) {
          ImVec2 popup_pos = ImGui::GetWindowPos();
          ImVec2 popup_size = ImGui::GetWindowSize();
          ImVec2 mouse_pos = ImGui::GetMousePos();
          bool inside_popup = mouse_pos.x >= popup_pos.x &&
                              mouse_pos.x <= popup_pos.x + popup_size.x &&
                              mouse_pos.y >= popup_pos.y &&
                              mouse_pos.y <= popup_pos.y + popup_size.y;
          if (ImGui::IsMouseClicked(0) && !inside_popup) {
        ...
        if (ui_state->builder_op == RFUI_OP_WITHIN) {
          ...
          ImGui::InputText("From", ui_state->builder_value,
                           sizeof(ui_state->builder_value));
          ImGui::SetNextItemWidth(100);
          ImGui::InputText("To", ui_state->builder_value2,
                           sizeof(ui_state->builder_value2));
        } else if (ui_state->builder_op == RFUI_OP_IN) {
          ImGui::SetNextItemWidth(200);
          ImGui::InputText("Values", ui_state->builder_value,
                           sizeof(ui_state->builder_value));
          ImGui::TextDisabled("Space-separated: 'AAPL 'MSFT 'GOOG");
        } else {
          ImGui::SetNextItemWidth(140);
          ImGui::InputText("Value", ui_state->builder_value,
                           sizeof(ui_state->builder_value));
        }
        ...
        if (ImGui::Button("Add")) {
          ...
          if (cr->num_rules < MAX_COL_RULES) {
            ...
            if (ui_state->builder_op == RFUI_OP_WITHIN) {
              ...
              strncpy(nr->value, ui_state->builder_value,
                      sizeof(nr->value) - 1);
        ...
        // --- Expression Rule Builder ---
        ImGui::TextDisabled("Rayfall expression (must return boolean vector):");
        ImGui::SetNextItemWidth(-1);
        ImGui::InputText("##expr", ui_state->builder_expr,
                         sizeof(ui_state->builder_expr));
        ImGui::TextDisabled("e.g. (> %s 800)  or  (== %s 'AAPL)", col_name,
                            col_name);

        // Add / Cancel
        if (ImGui::Button("Add")) {
          if (ui_state->builder_expr[0]) {
            rfui_col_rules_t *cr = &widget->col_rules[ci];
            if (cr->num_rules < MAX_COL_RULES) {
              rfui_rule_t *nr = &cr->rules[cr->num_rules++];
              nr->op = -1; // raw expression
              strncpy(nr->value, ui_state->builder_expr, sizeof(nr->value) - 1);
              nr->value[sizeof(nr->value) - 1] = '\0';
              nr->color = 0;
              nr->fn = nullptr;
              send_col_rules(widget, ci);
