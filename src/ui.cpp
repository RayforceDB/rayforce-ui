...
static char *get_config_path(void) {
  ...
  if (config_dir) {
    snprintf(config_dir, strlen(home) + strlen("/.config") + 1, "%s/.config",
             home);
...
i32_t rfui_ui_init(nil_t) {
  ...
  // On Windows/Linux, content scale reflects actual UI scaling (1.25, 1.5,
  // etc.) and both fonts and style need scaling.
  ...
  ImGui::GetStyle().HoverFlagsForTooltipMouse |=
      ImGuiHoveredFlags_DelayShort; // Reduce tooltip interference
  io.ConfigFlags |=
      ImGuiConfigFlags_NavEnableKeyboard;           // Enable Keyboard Controls
  io.ConfigFlags |= ImGuiConfigFlags_DockingEnable; // Enable Docking
  io.ConfigFlags |= ImGuiConfigFlags_ViewportsEnable; // Enable Multi-Viewport
  io.ConfigWindowsResizeFromEdges = true; // Enable Resize from edges
  io.ConfigWindowsMoveFromTitleBarOnly =
      true; // Enable Move from title bar only

  // Set up persistent layout file (ImGui auto-saves on shutdown, auto-loads on
  // startup)
  g_ini_path = get_config_path();
  if (g_ini_path) {
    io.IniFilename = g_ini_path;
  }
  // If get_config_path() fails, io.IniFilename remains "imgui.ini" (ImGui
  // default)
  ...
  io.Fonts->AddFontFromMemoryTTF((void *)embed_IBMPlexSans_Bold_ttf,
                                 embed_IBMPlexSans_Bold_ttf_len, font_size,
                                 &font_cfg);
  ...
  io.Fonts->AddFontFromMemoryTTF((void *)embed_fa_solid_900_otf,
                                 embed_fa_solid_900_otf_len, icon_size,
  ...
  io.Fonts->AddFontFromMemoryTTF((void *)embed_IBMPlexSans_Bold_ttf,
                                 embed_IBMPlexSans_Bold_ttf_len,
                                 large_font_size, &large_cfg);

  // Regular IBM Plex Sans for grid data cells (index 2)
  ImFontConfig regular_cfg;
  regular_cfg.FontDataOwnedByAtlas = false;
  io.Fonts->AddFontFromMemoryTTF((void *)embed_IBMPlexSans_Regular_ttf,
...
i32_t rfui_ui_run(nil_t) {
  ...
  while (!glfwWindowShouldClose(g_window) && !rfui_ctx_get_quit(g_ctx)) {
    ...
    // Poll events (VSync limits the loop rate via SwapBuffers)
    glfwPollEvents();
    ...
    if (g_ctx->ray_to_ui != nullptr) {
      while (messages_processed < MAX_MESSAGES_PER_FRAME &&
             (msg = (rfui_ray_msg_t *)rfui_queue_pop(g_ctx->ray_to_ui)) !=
                 nullptr) {
    ...
    // Main window: custom title bar + REPL content
    {
      ...
      // Revert to full-screen fixed window to avoid "double window" effect
      ImGui::SetNextWindowPos(vp->Pos);
      ImGui::SetNextWindowSize(vp->Size);
      ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
      ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f);
      ImGui::PushStyleColor(ImGuiCol_WindowBg,
                            ImVec4(0.051f, 0.067f, 0.090f, 0.85f));
      ...
      // Use Viewport size for title bar width to match full window
      ...
      // --- CRITICAL: OS Window Management (Resize & Move) ---
      // Since we are borderless (GLFW_DECORATED=false), we must handle this
      // manually for the OS window.

      // Window Resize (Manual Edge Detection)
      // Check for hover near edges to change cursor and handle resize
      // Only do this if not interacting with something else (like a floating
      // window or widget) AND if we are actually hovering the background window
      if (!ImGui::IsAnyItemActive() && ImGui::IsWindowHovered()) {
        const float BORDER_HIT = 7.0f;
        ImVec2 mp = ImGui::GetMousePos(); // ImGui coords (screen space usually)
        ImVec2 wp = vp->Pos;
        ImVec2 ws = vp->Size;

        bool left = mp.x < wp.x + BORDER_HIT;
        bool right = mp.x > wp.x + ws.x - BORDER_HIT;
        bool top = mp.y < wp.y + BORDER_HIT;
        bool bottom = mp.y > wp.y + ws.y - BORDER_HIT;
        ...
        if (left || right || top || bottom) {
          ...
          if (ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
            ...
            if (wh < 300)
              wh = 300;
            // Set Bounds
            glfwSetWindowPos(g_window, wx, wy);
            glfwSetWindowSize(g_window, ww, wh);
          }
        }
      }

      ImGui::SetCursorScreenPos(
          ImVec2(title_min.x + 12.0f,
                 title_min.y + (title_h - ImGui::GetFontSize()) * 0.5f));
      ImGui::PushStyleColor(ImGuiCol_Text,
                            ImVec4(0.914f, 0.627f, 0.200f, 1.0f));
      ...
      if (ImGui::Button(ICON_FOLDER_OPEN "##open", ImVec2(btn_w, title_h))) {
        ...
        ImGuiFileDialog::Instance()->OpenDialog("LoadScript", "Open Script",
                                                ".rfl", config);
      }
      ...
      if (ImGui::Button(maximized ? ICON_RESTORE "##max"
                                  : ICON_MAXIMIZE "##max",
      ...
      ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                            ImVec4(0.973f, 0.318f, 0.286f, 0.5f));
      ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                            ImVec4(0.973f, 0.318f, 0.286f, 0.8f));
      ...
      ImGui::InvisibleButton("##titlebar_drag",
                             ImVec2(btn_x - title_min.x, title_h));
      ...
      ImGui::GetWindowDrawList()->AddLine(
          ImVec2(title_min.x, title_max.y), ImVec2(title_max.x, title_max.y),
          IM_COL32(48, 54, 61, 255)); // COL_BORDER #30363D

      // --- File dialog ---
      if (ImGuiFileDialog::Instance()->Display(
              "LoadScript", ImGuiWindowFlags_NoCollapse, ImVec2(800, 500))) {
    ...
    // REPL autocomplete overlay (on top of widgets)
    rfui_repl_render_overlay();
    ...
    // Multi-viewport: update and render platform windows (always, even if main
    // minimized)
    ImGuiIO &io = ImGui::GetIO();
...
nil_t rfui_ui_wake(nil_t) { glfwPostEmptyEvent(); }
